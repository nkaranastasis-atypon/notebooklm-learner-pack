...
- **5 Knowledge Tests** — NotebookLM Flashcards and Quiz prompts spanning recall, application, diagnosis, synthesis, and judgment

Every prompt has **6 audience-lens variants** that the user toggles live in the browser: ELI5, Newbie, Clinical, Operator (default), Finance, Deep Dive.
...
- **Brand voice** (required) — Techy Surgeon, Duke Health, RevelAi Modern Enterprise, or custom. Brand determines voice card, colors, typography, and the imperative voice woven into every prompt.
...
- **Audience priorities** (optional) — if the user flags a primary audience (e.g., "this is for clinical leaders"), default the lens toggle to that audience.
...
This is the most important step. The prompts must be **validated and grounded in the source material** rather than generic. A prompt that says "compare two entities from the source" is weaker than "compare TEAM and CJR-X side-by-side — 5-7 operationally consequential parameters each."
...
See `references/html-template.md` for the template structure, the data-driven JavaScript pattern, and the worked CJR-X + Techy Surgeon example.
...
Save the HTML to `/sessions/eloquent-happy-archimedes/mnt/RevelAi Health/NotebookLM Learner Pack/one-pagers/` with a filename pattern: `{brand-slug}-{topic-slug}-learner-pack.html`.
...
3. **Brand voice check** — The voice card at the top of the HTML should match the brand's established voice. The `OUTRO` block injected into every prompt should carry the same voice markers (e.g., "Techy Surgeon — analytical, direct" vs. "Duke Health — academic, rigorous" vs. "RevelAi Modern Enterprise — operator-focused, enterprise-grade").
...
- **Don't put gravitas openings in the prompts.** No "In the quiet corridors of American healthcare..." No staccato fragments for emphasis. Direct, operator-focused framing.
...
- `references/brand-voices.md` — Voice cards, colors, typography for Techy Surgeon, Duke, RevelAi ModEnt, and generic/custom brands
